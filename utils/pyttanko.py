...
-------------------------------------------------------------------
DISCLAIMER:
    - This is an edited version of pyttanko for Kisumi server
    instance use, remember it's not a real pyttanko so use on your own.
"""

__author__ = "Franc[e]sco <lolisamurai@tfwno.gf>, Lenforiee <lenforiee@misumi.me>"
__version__ = "2.2.0"
...
def mods_str(mods):
    ...
    if mods & MODS_RX != 0: res += "RX"
    if mods & MODS_AP != 0: res += "AP"
...
def mods_from_str(string):
    ...
    while string != "":
        ...
        elif string.startswith("RX"): res |= MODS_RX
        elif string.startswith("AP"): res |= MODS_AP
        else:
            string = string[1:]
            continue
...
def ppv2(
    ...
    if mods & MODS_RX:
        if ar > 10.67:
            ar_bonus += pow(ar - 10.67, 1.75)
        elif ar < 9.5:
            ar_bonus += 0.05 * (9.5 - ar)
    else:
        if ar > 10.33:
