...
async def osuSubmitModularSelector(conn: Connection) -> Optional[bytes]:
    ...
    elif s.pp > 400 and not bypass and not s.mods & Mods.RELAX and s.mode == 0 and s.passed:
        webhook_url = glob.config.webhooks['audit-log']
        webhook = Webhook(url=webhook_url)
        embed = Embed(title = f'')
        embed.set_author(url = f"https://{glob.config.domain}/u/1", name = 'Anticheat', icon_url = f"https://a.{glob.config.domain}/1")
    ...
    s.id = await glob.db.execute(
        ...
        '%s, %s, %s, %s, %s)', [
            ...
            s.time_elapsed, s.client_flags, s.player.id, s.perfect, to_readable(int(s.mods))
        ]
    )
...
async def getScores(p: 'Player', conn: Connection) -> Optional[bytes]:
    ...
    query = [
        ...
        "WHERE s.map_md5 = %s AND s.status = 2 AND (u.priv & 1 OR u.id = %s) AND mode = %s"
    ]
